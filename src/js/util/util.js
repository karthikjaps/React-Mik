...
export function removeQueryStringParam(uri, key) {
  let tmpQueryString = "";

  let prefix = encodeURIComponent(key) + "=";
  let pars = uri.replace(/\?/g, "").split(/[&;]/g);
  ...
  tmpQueryString = pars.length > 0 ? `?${pars.join("&")}` : "";
...
export function stringToObject(path, value, obj) {
